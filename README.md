...
SPA
SPA - Single page application (Всё происходит в одном HTML файле, который приходит пользователю)
...
позволяет использовать route (изначально всё нужно обвернуть в один блок под названием BrowserRouter. Он позволяет отрисовывать компоненты, отслеживания их url ( react <Route path='/profile' component={Profile} />​ ). Чтобы задать изменение url нужно использовать тег NavLink (
...
Удаляем файл StoreContext.js. Он нам больше не нужен.

Удаляем функцию reRenderEntireTree в index.js! Она нам также больше не нужна.

Компоненту Provider оставляем, но импортируем её уже не через StoreContext, а через react-redux.
...
Отправляются 2 запроса на сервер.

Так получилось что запрос на получения данных о профиле пришёл РАНЬШЕ.

Отрисовывается компонента за счёт полученных данных.

<p> остаётся пустым (или же просто заглушкой), в локальный state ничего не приходит, значит <input /> неоткуда брать данные, он также остаётся пустым.

Приходят данные по второму запросу(статуса)

В <p> приходят пропсы и отрисовывается статус.

В локальный state приходят данные, НО <input /> уже отрисовался, а значит он не будет вновь брать данные из локального state, он так и останется пустым.
...
Устанавливаем пакет Redux-Form

Создаём новую ветку state под названием form и присваиваем ей formReducer, импортированный из пакета redux-form ( В самом пакете он называется reducer, что может ввести в заблуждение, так что импортируйте под названием, которое будет понятно ВАМ, например formReducer)
...
export default store;
window.store = store;
Для использования redux-form создаёте компоненту
...
Оборачиваете созданную компоненту HOC'ом под названием reduxForm, также импортированным из redux-form
...
Сбор данных с формы по нажатию на кнопку.
...
immutability (имьютабельность, неизменяемость) - входные данные, пришедшие в функцию, эта функция не должна менять (речь про объекты и массивы, так как по ссылке они передаются, поэтому делаем копию)
...
детерменированность\идемпотентность - сколько бы раз на вход чистой функции не подавали одно и тоже, на выходе чистой функции должен быть один и тот же результат
чистая функция должна вернуть (return) что-либо
